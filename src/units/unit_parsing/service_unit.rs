...
pub fn parse_service(
    ...
    let uid = if let Some(user) = &service_config.exec_section.user {
    ...
    let gid = if let Some(group) = &service_config.exec_section.group {
    ...
    for group in &service_config.exec_section.supplementary_groups {
