...
pub fn sanity_check_dependencies(unit_table: &HashMap<UnitId, Unit>) -> Result<(), String> {
        ...
        let root_id = if not_finished_ids.len() == 0 {
            ...
            if let Some(id) = root_id {
                *id
                ...
                return Err(format!("There is a cycle"));
            }
        };
...
fn search_backedge(
    ...
    if visited_ids.contains(id) {
        let mut err_str = String::from("There is a cycle between units:");
        for idx in 0..visited_ids.len() - 1 {
            err_str.push('\n');
            let name1 = unit_table.get(&visited_ids[idx]).unwrap().conf.name();
            let name2 = unit_table.get(&visited_ids[idx]).unwrap().conf.name();
            err_str.push_str(&format!(", {} -before-> {}", name1, name2));
        }
        let name1 = unit_table
            .get(&visited_ids[visited_ids.len() - 1])
            .unwrap()
            .conf
            .name();
        let name2 = unit_table
            .get(&visited_ids[visited_ids.len()])
            .unwrap()
            .conf
            .name();
        err_str.push_str(&format!("{} -before-> {}", name1, name2));
